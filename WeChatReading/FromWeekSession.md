### 2021-03-19&21
#### [Weak 1](https://mp.weixin.qq.com/s?__biz=MzUzNjg3NzI0Mg==&mid=2247484333&idx=1&sn=8fb17f346d6c53ea2916deb455c199cc&chksm=faeec278cd994b6eb6a14a1a3c01d4f708784faafb7be3f1c1ce147d92cc1f2550b68c71c971&mpshare=1&scene=1&srcid=0319YPxK5QhITGfmfh708rhj&sharer_sharetime=1616142683269&sharer_shareid=dad85592c4a6617e83e9a87f69e0fb4d&key=&ascene=1&uin=NzE4NTkwNDg1&devicetype=Windows+10+x64&version=63000039&lang=zh_CN&exportkey=Ax1jt%2Bua92Jn3aGX%2FcSt5nY%3D&pass_ticket=O%2BbMwDOeG1k%2B6FGntZDh2IT7JERzSoGz%2F8hc36ETOSuTCHP95g%2B0bmiMkIF5LzTg&wx_header=0)
:maple_leaf: [解码 编码-检索权衡 以 预测 对重合事件的记忆](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6765409/)
##### 摘要
新旧事件重合时，需要在编码这两个事件中取得平衡，在记忆中这种重合无处不在，因此这种编码对情景记忆的计算模型很重要(O'Reilly & McClelland 1994; Hasselmo 2005)。但是，之前的研究并没有直接将 编码-检索的平衡 与 重合事件的记忆方式 这两者建立连接。本研究对被试进行EEG解码，发现平衡反映于神经活动的分布模式，并且这种模式能预测重合事件在之后的记忆方式。

也就是说，如果神经模式 偏向编码 或者 远离检索， 那 重合的新事件 很有可能 会随后被记住。并且，编码或检索状态下的神经标记 独立于 以前研究的 对后续记忆的EEG预测因子。取而代之的是，我们证明，与记忆的编码相比，任务本身能更好解释 以前研究的 对后续记忆的EEG预测因子。

总的来说，研究有助于理解记忆系统如何平衡编码和检索状态，更广的说，有助于我们理解 记忆的成功形成的神经机制。
##### 目的
研究新旧事件发生重合时，大脑内部的编码与提取之间的平衡机制
##### 方法
EEG记录头皮电位变化，MVPA解码
##### 任务
...
2. 平衡机制可以预测新旧事件重合部分是如何被大脑记住的

3. 编码与提取状态的神经活动是独立于这种预测机制的

4. 实验任务而非记忆编码，可以很好地解释后续记忆的EEG预测机制
...
##### 方法
使用一种可逆性的灭活剂抑制PPC的活动，以此来比较运动任务（MDD）和视觉任务（MDC）做决定时表现的异同
##### 任务
...
##### 摘要
This approach revealed two opposing gene expression gradients: one that contains a series of genes with expression magnitudes that ascend from posterior (e.g., areas human occipital [hOc]1, hOc2, hOc3, etc.) to anterior cytoarchitectonic areas (e.g., areas fusiform gyrus [FG]1-FG4) and another that contains a separate series of genes that show a descending gradient from posterior to anterior areas. Using data from the living human brain, we show that each of these gradients correlates strongly with variations in measures related to either thickness or myelination of cortex, respectively. We further reveal that these genetic gradients emerge along unique trajectories in human development: the ascending gradient is present at 10-12 gestational weeks, while the descending gradient emerges later (19-24 gestational weeks). Interestingly, it is not until early childhood (before 5 years of age) that the two expression gradients achieve their adult-like mean expression values. Additional analyses in nonhuman primates (NHPs) reveal that homologous genes do not generate the same ascending and descending expression gradients as in humans. We discuss these findings relative to previously proposed hierarchies based on functional and cytoarchitectonic features of visual cortex. Altogether, these findings bridge macroscopic features of human cytoarchitectonic areas in visual cortex with microscopic features of cellular organization and genetic expression, which, despite the complexity of this multiscale correspondence, can be described by a sparse subset (approximately 200) of genes. These findings help pinpoint the genes contributing to healthy cortical development and explicate the cortical biology distinguishing humans from other primates, as well as establishing essential groundwork for understanding future work linking genetic mutations with the function and development of the human brain.

人类视觉皮层的组织结构具有惊人的一致性。然而最近的发现表明，相对于人类视觉皮层的折叠，功能性和细胞结构性区域之间存在意想不到的耦合，但将皮层的这些解剖和功能特征联系起来的统一原理仍未知。为了填补这一空白，我们结合了对细胞结构区域的独立和[Ground Truth（真值、真实的有效值或者是标准的答案）](https://zh.wikipedia.org/zh/Ground_truth) 的测量和人类枕颞皮层内的遗传组织表征。使用数据驱动的方法，我们检查了在细胞结构间不同的基因表达是否有助于将枕颞皮层区域化为基于转录组学的层次结构。
<div align=center><img src="https://www.ncbi.nlm.nih.gov/corecgi/tileshop/tileshop.fcgi?p=PMC3&id=273440&s=90&r=1&c=3"  height="232px" width="509px"  ></div>
这种方法揭示了两个相反的基因表达梯度：一个包含一系列表达幅度从后部（例如人枕[hOc] 1，hOc2，hOc3等）到前细胞建筑区域（例如梭形区域）上升的基因。 gyrus [FG] 1-FG4）和另一个包含单独的一系列基因，这些基因显示从后部到前部的下降梯度。使用来自活人脑的数据，我们显示出这些梯度中的每一个都分别与皮质厚度或髓鞘形成相关的量度变化密切相关。我们进一步揭示了这些遗传梯度沿着人类发展的独特轨迹出现：上升梯度出现在10-12个孕周，而下降梯度出现在以后（19-24个孕周）。有趣的是，直到儿童早期（5岁之前），这两个表达梯度才达到其成年人般的平均表达值。
在非人类灵长类动物（NHP）中进行的其他分析表明，同源基因不会产生与人类相同的上升和下降表达梯度。我们讨论这些发现相对于以前提出的基于视觉皮层的功能和细胞结构特征的层次结构。总而言之，这些发现桥接了视觉皮层中人类细胞建筑区域的宏观特征与细胞组织和遗传表达的微观特征，尽管这种多尺度对应关系很复杂，但可以用基因的稀疏子集（约200个）来描述。这些发现有助于查明有助于健康皮质发育的基因，并阐明使人类与其他灵长类动物区分开的皮质生物学，以及为理解将遗传突变与人脑功能和发育联系起来的未来工作奠定重要基础。
##### 目的
探索视觉皮层的功能和细胞结构区之间一致性的连接机制
##### 方法
采用独立并可靠的方法测量了枕颞皮层细胞结构区和基因组织特征。使用数据驱动的方式检测细胞结构区不同的基因表的方式是否可以促进枕颞皮层的区域分层。
##### 任务
人类枕颞皮层有两个相关的基因表达梯度：一个是保持了一系列表达重要性的基因，它从后部（如枕叶的[hOc]1, hOc2, hOc3）到前部的细胞结构（如梭状回区域[FG]1-FG4）是呈下降的趋势；另一个是包含了一系列独立的基因，它从后部到前部是呈上升的趋势。

进一步揭示了这些基因梯度是沿着人类成长过程中独特的轨道出现的：上升梯度出现在10-12妊娠周，下降梯度出现在后期（19-24妊娠周）。有趣的是，直到早期童年期（在5岁之前）两个基因表达梯度才达到类似于成人的平均表达值。另外在非人类的灵长类动物身上的实验结果分析显示，对应的基因并不会产生和人类同样的上升和下降基因表达梯度。
##### 结果
这些发现帮助我们指出对健康的皮层发展和可区分人类和其他灵长类动物的皮层生物学提供了证据，同时也为理解未来与人脑生长过程中发生的基因突变相关研究提供了重要的基础工作。
